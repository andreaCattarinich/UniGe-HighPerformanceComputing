...
int main()
{
  ...
  cudaMalloc((void**)&d_temp1, sizeof(float) * size);
  cudaMalloc((void**)&d_temp2, sizeof(float) * size);

  // Transfer data from host to device memory
  cudaMemcpy(d_temp1, temp1, sizeof(float) * size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_temp2, temp2, sizeof(float) * size, cudaMemcpyHostToDevice);
  ...
  dim3 threadsPerBlock(16,16); // blocco 16×16 thread
  dim3 numBlocks((ni+15)/16, (nj+15)/16); // griglia sufficiente a coprire la matrice
  ...
  cudaMemcpy(temp1, d_temp1, sizeof(float) * size, cudaMemcpyDeviceToHost);
  cudaMemcpy(temp2, d_temp2, sizeof(float) * size, cudaMemcpyDeviceToHost);
