...
\section{OpenMP}
\label{sec:OpenMP}
All results were obtained by compiling with the following command (see \texttt{Makefile})

\begin{verbatim}
    icpx -O2 -march=native -fopenmp 
\end{verbatim}

\noindent
Running each version 10 times, and computing the average runtime.
...
\noindent
For omp2, guided scheduling provides clear improvements in both speedup and efficiency. The measured speedup remains close to the theoretical value up to 16 threads (theoretical speedup 16, observed ≈13), corresponding to an efficiency of about 80\%. This demonstrates that guided scheduling effectively mitigates the workload irregularity.
